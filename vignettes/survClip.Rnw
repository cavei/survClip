...
Many times it is of utmost importance to find genes that are prognostic markers. What about finding molecular circuits (circuits defined as small clusters of genes of the same pathway)?
That why we devised \textit{survClip}. The package include 2 main functions that allow to predict if a given pathway is relevant for patients' survival and which is the best candidate circuit.
...
We are ready to run the pathway analysis. In this case we choose to evaluate the first three pricipal components. The pcsSurvCoxMethod argument is in the default set to "regular" which means that a normal PCA is done. In this example we use the 'topological' method. In this case the topology of the pathway is taken into account when computing PCs.
<<runPathwaySurvClip>>=
